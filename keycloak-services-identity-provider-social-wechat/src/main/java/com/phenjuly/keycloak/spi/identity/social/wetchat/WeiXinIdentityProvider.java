 ...
 */
package com.phenjuly.keycloak.spi.identity.social.wetchat;
...
public class WeiXinIdentityProvider extends AbstractOAuth2IdentityProvider<OAuth2IdentityProviderConfig>
    ...
    // PARAMETER_
    public static final String OAUTH2_CLIENT_ID_KEY = "appid";
    public static final String OAUTH2_CLIENT_SECRET_KEY = "secret";

    public static final String WECHAT_APP_ID_KEY = "clientId2";
    public static final String WECHAT_APP_SECRET_KEY = "clientSecret2";
    ...
    public BrokeredIdentityContext getFederatedIdentity(String response, boolean wechat) {
        ...
        try {
            ...
            if (wechat) {
                ...
                String url = PROFILE_URL.replace("ACCESS_TOKEN", accessToken).replace(OPENID.toUpperCase(), openid);
    ...
    private boolean isWechatBrowser(String ua) {
        String wechatAppId = getConfig().getConfig().get(WECHAT_APP_ID_KEY);
        String wechantSecret = getConfig().getConfig().get(WECHAT_APP_SECRET_KEY);
    ...
    protected UriBuilder createAuthorizationUrl(AuthenticationRequest request) {
        ...
        if (isWechatBrowser(ua)) {
            ...
            uriBuilder.queryParam(OAUTH2_PARAMETER_SCOPE, WECHAT_DEFAULT_SCOPE)
                    ...
                    .queryParam(OAUTH2_CLIENT_ID_KEY, getConfig().getConfig().get(WECHAT_APP_ID_KEY))
            ...
            uriBuilder.queryParam(OAUTH2_PARAMETER_SCOPE, getConfig().getDefaultScope())
                    .queryParam(OAUTH2_PARAMETER_STATE, request.getState().getEncoded())
                    .queryParam(OAUTH2_CLIENT_ID_KEY, getConfig().getClientId())
                    .queryParam(OAUTH2_PARAMETER_REDIRECT_URI, request.getRedirectUri());
        }
    ...
    protected class Endpoint {
        ...
        public SimpleHttp generateTokenRequest(String authorizationCode) {
            return SimpleHttp.doPost(getConfig().getTokenUrl(), session).param(OAUTH2_PARAMETER_CODE, authorizationCode)
                    .param(OAUTH2_CLIENT_ID_KEY, getConfig().getClientId())
                    .param(OAUTH2_CLIENT_SECRET_KEY, getConfig().getClientSecret())
        ...
        public SimpleHttp generateTokenRequest(String authorizationCode, boolean wechat) {
            if (wechat) {
                return SimpleHttp.doPost(WECHAT_TOKEN_URL, session)
                        ...
                        .param(OAUTH2_CLIENT_ID_KEY, getConfig().getConfig().get(WECHAT_APP_ID_KEY))
                        .param(OAUTH2_CLIENT_SECRET_KEY, getConfig().getConfig().get(WECHAT_APP_SECRET_KEY))
                        .param(OAUTH2_PARAMETER_REDIRECT_URI, uriInfo.getAbsolutePath().toString())
                        .param(OAUTH2_PARAMETER_GRANT_TYPE, OAUTH2_GRANT_TYPE_AUTHORIZATION_CODE);
            }
            return SimpleHttp.doPost(getConfig().getTokenUrl(), session).param(OAUTH2_PARAMETER_CODE, authorizationCode)
